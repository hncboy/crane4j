...
参照 Spring 的生命周期回调机制，`crane4j` 也提供了特殊的回调接口 `ContainerRegisterAware`，该接口类似 `Spring` 的 `BeanPostProcessor`，允许用户感知并干涉数据源容器的注册行为。

比如，默认提供的 `DefaultCacheableContainerProcessor` 便实现了该接口，在用户的容器注册前，将会把要注册的容器对象包装并替换具备缓存功能的增强容器，`ContainerRegisterLogger` 则会在容器注册后，输出注册信息。
