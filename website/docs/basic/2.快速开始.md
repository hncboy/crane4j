...
然后创建**两个容器**，并将其注册到 `crane4j` 全局配置类中：

~~~java
@PostConstruct
public void init() {
    // 通过 Spring 上下文获取 Crane4jApplicationContext
    Crane4jApplicationContext context = SpringUtil.getBean(Crane4jApplicationContext.class);

    // 注册班级信息的数据源
    Container<Integer> studentClassContainer = LambdaContainer.forLambda(
        "student-class", ids -> ids.stream().collect(Collectors.toMap(
            Fucntion.identity, id -> new StudentClass(id, "一年" + id + "班")
        ))
    );
    context.registerContainer(studentClassContainer);
...
- 除了上述的编码式注册外，也支持通过更多种途径创建并注册数据源类型的容器，具体参考后文[数据源容器](./../advance/1.数据源容器.md)一节。
